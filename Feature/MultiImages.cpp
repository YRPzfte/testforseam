...
MultiImages::MultiImages(const string& source_dir, const string& target_dir, const string& view,
                         LINES_FILTER_FUNC * _width_filter,
                         LINES_FILTER_FUNC * _length_filter) : parameter(source_dir, target_dir, view) {

    for(int i = 0; i < parameter.image_file_full_names.size(); ++i) {
#ifndef NDEBUG
        images_data.emplace_back(parameter.image_file_full_names[i],
                                 _width_filter,
                                 _length_filter,
                                 &parameter.debug_dir);
#else
    ...
    ostringstream source_depth_oss;
    source_depth_oss << parameter.source_dir <<  "/depth.png";
    Mat source_depth = imread(source_depth_oss.str().c_str(), IMREAD_UNCHANGED);// depth

    float original_source_size = source_depth.rows * source_depth.cols;
    if(original_source_size > DOWN_SAMPLE_IMAGE_SIZE) {
        scale = sqrt(DOWN_SAMPLE_IMAGE_SIZE / original_source_size);
        resize(source_depth, source_depth, Size(), scale, scale);
    }

    depth_images.push_back(source_depth);

    ostringstream target_depth_oss;
    target_depth_oss << parameter.target_dir <<  "/depth.png";
    Mat target_depth = imread(target_depth_oss.str().c_str(), IMREAD_UNCHANGED);// depth

    float original_target_size = target_depth.rows * target_depth.cols;
    if(original_target_size > DOWN_SAMPLE_IMAGE_SIZE) {
        scale = sqrt(DOWN_SAMPLE_IMAGE_SIZE / original_target_size);
        resize(target_depth, target_depth, Size(), scale, scale);
    } else {
        scale = 1;
    }

    depth_images.push_back(target_depth);

}

void MultiImages::update_images_data(string source_path, string target_path, LINES_FILTER_FUNC * _width_filter, LINES_FILTER_FUNC * _length_filter) {
    images_data[0] = ImageData(source_path, _width_filter, _length_filter);
    images_data[1] = ImageData(target_path, _width_filter, _length_filter);
}
...
const vector<cv::detail::ImageFeatures> & MultiImages::getImagesFeaturesByMatchingPoints() const {
...
const vector<cv::detail::MatchesInfo> & MultiImages::getPairwiseMatchesByMatchingPoints() const {
...
const vector<cv::detail::CameraParams> & MultiImages::getCameraParams() const {
    if(camera_params.empty()) {
            ...
            for(int j = 0; j < images_data.size(); ++j) {
                for(int k = 0; k < apap_overlap_mask[i][j].size(); ++k) {
                    if(apap_overlap_mask[i][j][k]) {
                        ...
                        cv::detail::focalsFromHomography(H / H.at<double>(2, 2),
                                                     f0, f1, f0_ok, f1_ok);
                        if(f0_ok && f1_ok) {
        ...
        const vector<cv::detail::ImageFeatures> & images_features    = getImagesFeaturesByMatchingPoints();
        const vector<cv::detail::MatchesInfo>   & pairwise_matches   = getPairwiseMatchesByMatchingPoints();
        const vector<pair<int, int> > & images_match_graph_pair_list = parameter.getImagesMatchGraphPairList();
        for(int i = 0; i < images_match_graph_pair_list.size(); ++i) {
            const pair<int, int> & match_pair = images_match_graph_pair_list[i];
            const int & m1 = match_pair.first, & m2 = match_pair.second;
            const int m_index = m1 * (int)images_data.size() + m2;
            const cv::detail::MatchesInfo & matches_info = pairwise_matches[m_index];
        ...
        Ptr<cv::detail::BundleAdjusterBase> adjuster = makePtr<cv::detail::BundleAdjusterReproj>();
        ...
        if(WAVE_CORRECT != WAVE_X) {
            ...
            waveCorrect(rotations, ((WAVE_CORRECT == WAVE_H) ? cv::detail::WAVE_CORRECT_HORIZ : cv::detail::WAVE_CORRECT_VERT));
            for(int i = 0; i < camera_params.size(); ++i) {
...
const vector<SimilarityElements> & MultiImages::getImagesSimilarityElements(const enum GLOBAL_ROTATION_METHODS & _global_rotation_method) const {
    ...
    if(result.empty()) {
        ...
        const vector<cv::detail::CameraParams> & camera_params = getCameraParams();
        for(int i = 0; i < images_data.size(); ++i) {
...
const vector<pair<Point2, Point2> >& MultiImages::getFeaturePairMatches() const {
    if(feature_pair_matches.empty()) {
        const vector<vector<vector<pair<int, int> > > > & feature_pairs = getFeaturePairs();
        const vector<pair<int, int> > & images_match_graph_pair_list = parameter.getImagesMatchGraphPairList();
        assert(images_match_graph_pair_list.size() == 1);
        const pair<int, int> & match_pair = images_match_graph_pair_list[0];
        const int & m1 = match_pair.first, & m2 = match_pair.second;
        const vector<Point2> & m1_fpts = images_data[m1].getFeaturePoints();
        const vector<Point2> & m2_fpts = images_data[m2].getFeaturePoints();
        for(int j = 0; j < feature_pairs[m1][m2].size(); ++j) {
            feature_pair_matches.push_back(make_pair(m1_fpts[feature_pairs[m1][m2][j].first], m2_fpts[feature_pairs[m1][m2][j].second]));
        }
    }
    return feature_pair_matches;
}
...
void MultiImages::textureMapping(const vector<vector<Point2> > & _vertices,
                                const Size2 & _target_size,
                                const BLENDING_METHODS & _blend_method) const{
    vector<Mat> warp_images;
    textureMapping(_vertices, _target_size, _blend_method, warp_images);
}
...
void MultiImages::textureMapping(const vector<vector<Point2> > & _vertices,
    ...
    for(int i = 0; i < images_data.size(); ++i) {
        ...
        Mat image = Mat::zeros(rects[i].height + shift.y, rects[i].width + shift.x, CV_8UC3);
        Mat depth = Mat::zeros(rects[i].height + shift.y, rects[i].width + shift.x, CV_16UC1);
        Mat w_mask = (_blend_method != BLEND_AVERAGE) ? Mat::zeros(image.size(), CV_32FC1) : Mat();
        vector<pair<Point2, Point2> > mapping_pair;
                ...
                if(polygon_index != NO_GRID) {
                    ...
                    if(p_f.x >= 0 && p_f.y >= 0 &&
                       ...
                       p_f.y <= images_data[i].img.rows) {
                        ...
                        image.at<Vec3b>(y, x) = Vec3b(c[0], c[1], c[2]);
                        depth.ptr<ushort>(y)[x] = depth_images[i].ptr<ushort>(int(p_f.y))[int(p_f.x)];
                        mapping_pair.push_back(make_pair(Point2(int(p_f.x + 1), int(p_f.y + 1)), Point2(int(rects[i].x + x + 1), int(rects[i].y + y + 1))));
        ...
        mapping_pairs.push_back(mapping_pair);
        _warp_images.emplace_back(image);
        _warp_depths.emplace_back(depth);
        origins.emplace_back(rects[i].x, rects[i].y);
        if(_blend_method != BLEND_AVERAGE) {
            new_weight_mask.emplace_back(w_mask);
        }
    }
    assert(origins.size() == 2 && _warp_images.size() == 2);

    source_warping = Mat::zeros(round(_target_size.height), round(_target_size.width), CV_8UC3);
    Mat source_imageROI = source_warping(Rect(origins[0].x, origins[0].y, _warp_images[0].cols, _warp_images[0].rows));
    _warp_images[0].copyTo(source_imageROI);

    source_depth = Mat::zeros(round(_target_size.height), round(_target_size.width), CV_16UC1);
    Mat source_depth_imageROI = source_depth(Rect(origins[0].x, origins[0].y, _warp_depths[0].cols, _warp_depths[0].rows));
    _warp_depths[0].copyTo(source_depth_imageROI);


    target_warping = Mat::zeros(round(_target_size.height), round(_target_size.width), CV_8UC3);
    Mat target_imageROI = target_warping(Rect(origins[1].x, origins[1].y, _warp_images[1].cols, _warp_images[1].rows));
    _warp_images[1].copyTo(target_imageROI);

    target_depth = Mat::zeros(round(_target_size.height), round(_target_size.width), CV_16UC1);
    Mat target_depth_imageROI = target_depth(Rect(origins[1].x, origins[1].y, _warp_depths[1].cols, _warp_depths[1].rows));
    _warp_depths[1].copyTo(target_depth_imageROI);
}
