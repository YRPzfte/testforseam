...
Parameter::Parameter(const string& _source_dir, const string& _target_dir, const string& view) {
    source_dir = _source_dir;
    target_dir = _target_dir;
    file_name = "stereo_image";
    file_dir = ".";
    result_dir = "./build/result/";

    mkdir(result_dir.c_str(), S_IRWXU | S_IRGRP | S_IXGRP | S_IROTH | S_IXOTH);
#ifndef NDEBUG
    debug_dir = "./build/debugs/";
    mkdir(debug_dir.c_str(), S_IRWXU | S_IRGRP | S_IXGRP | S_IROTH | S_IXOTH);
#endif

    ostringstream source_oss, target_oss;
    source_oss << source_dir << "/" << view << ".png";
    target_oss << target_dir << "/" << view << ".png";

    image_file_full_names.push_back(source_oss.str().c_str());
    image_file_full_names.push_back(target_oss.str().c_str());
    ...
    images_match_graph_manually[0][1] = true;
