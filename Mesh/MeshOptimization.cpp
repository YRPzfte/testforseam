...
MeshOptimization::MeshOptimization(const MultiImages & _multi_images) {
    ...
    disparity_equation = make_pair(0, 0);
...
void MeshOptimization::setWeightToDisparityTerm(const double _weight) {
    disparity_weight = _weight;
}
...
void MeshOptimization::setWeightToSimilarityTerm(const double _weight,
                                                       const enum GLOBAL_ROTATION_METHODS _global_rotation_method) {
    local_similarity_weight = _weight;
    global_rotation_method         = _global_rotation_method;
}
...
double MeshOptimization::getDisparityTermWeight() const {
    return disparity_weight;
}
...
void MeshOptimization::reserveDataForConsistent(vector<Triplet<double> > & _triplets,
    ...
    alignment_equation.second = _align_equation * DIMENSION_2D;
    equation += alignment_equation.second;

    disparity_equation.first = equation;
    disparity_equation.second = _disparity_equation;
    equation += disparity_equation.second;

    const bool local_similarity_term = local_similarity_weight;
    int edge_count = (local_similarity_term) ? getEdgesCount() : 0;
    ...
    _triplets.reserve(alignment_equation.second * 8 + disparity_equation.second * 4 + (local_similarity_term) * (edge_neighbor_vertices_count * 8 + edge_count * 4) + _start_index);
    _b_vector.reserve(alignment_equation.second * 2 + disparity_equation.second * 1 + _start_index);
}
...
//void MeshOptimization::prepareAlignmentTerm(vector<Triplet<double> > & _triplets, vector<pair<int, double> > & _b_vector) const {
//
//    if(alignment_equation.second) {
//        const int equation = alignment_equation.first;
//        const vector<vector<InterpolateVertex> > & mesh_interpolate_vertex_of_matching_pts = multi_images->getInterpolateVerticesOfMatchingPoints();
//        const vector<detail::MatchesInfo> & pairwise_matches = multi_images->getPairwiseMatchesByMatchingPoints();
//        const vector<detail::ImageFeatures>& images_features = multi_images->getImagesFeaturesByMatchingPoints();
//        const vector<pair<int, int> > & images_match_graph_pair_list = multi_images->parameter.getImagesMatchGraphPairList();
//        const vector<int> & images_vertices_start_index = multi_images->getImagesVerticesStartIndex();
//        int eq_count = 0;
//
//        Mat source_img = multi_images->images_data[0].img;
//        Mat target_img = multi_images->images_data[1].img;
//
//        const pair<int, int> & match_pair = images_match_graph_pair_list[0];
//        const int & m1 = match_pair.first, & m2 = match_pair.second;
//        const int pm_index = m1 * (int)multi_images->images_data.size() + m2;
//
//        Mat inlier_img = Mat::zeros(source_img.rows, source_img.cols * 2, CV_8UC3);
//
//        Mat source_imageROI = inlier_img(Rect(0, 0, source_img.cols, source_img.rows));
//        source_img.copyTo(source_imageROI);
//        Mat target_imageROI = inlier_img(Rect(source_img.cols, 0, target_img.cols, target_img.rows));
//        target_img.copyTo(target_imageROI);
//
//        for(int j = 0; j < pairwise_matches[pm_index].matches.size(); ++j) {
//            const DMatch & D_Match = pairwise_matches[pm_index].matches[j];
//            Point2 source_p(images_features[m1].keypoints[D_Match.queryIdx].pt.x, images_features[m1].keypoints[D_Match.queryIdx].pt.y);
//            Point2 target_p(images_features[m2].keypoints[D_Match.trainIdx].pt.x, images_features[m2].keypoints[D_Match.trainIdx].pt.y);
//            circle(inlier_img, Point(source_p.x, source_p.y), 2, Scalar(255, 0, 0));
//            circle(inlier_img, Point(target_p.x + source_img.cols, target_p.y), 2, Scalar(255, 0, 0));
//            line(inlier_img, Point(source_p.x, source_p.y), Point(target_p.x + source_img.cols, target_p.y), Scalar(0,255,0));
//        }
//        imwrite("inlier_img.png", inlier_img);
//
//        for(int i = 0; i < images_match_graph_pair_list.size(); ++i) {
//            const pair<int, int> & match_pair = images_match_graph_pair_list[i];
//            const int & m1 = match_pair.first, & m2 = match_pair.second;
...
//                    }
//                }
//                _b_vector.emplace_back(equation + eq_count + 0, images_features[m2].keypoints[D_Match.trainIdx].pt.x);
//                _b_vector.emplace_back(equation + eq_count + 1, images_features[m2].keypoints[D_Match.trainIdx].pt.y);
//                eq_count += DIMENSION_2D;
//                for(int dim = 0; dim < DIMENSION_2D; ++dim) {
//                    for(int k = 0; k < multi_images->images_data[m2].mesh_2d->getPolygonVerticesCount(); ++k) {
//                        _triplets.emplace_back(equation + eq_count + dim,
...
//                }
//
//                _b_vector.emplace_back(equation + eq_count + 0, images_features[m1].keypoints[D_Match.queryIdx].pt.x);
//                _b_vector.emplace_back(equation + eq_count + 1, images_features[m1].keypoints[D_Match.queryIdx].pt.y);
//                eq_count += DIMENSION_2D;
//            }
//        }
//        assert(eq_count == alignment_equation.second);
